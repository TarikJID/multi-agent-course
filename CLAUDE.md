...
> Note: all six modules now have the full `study-material/` set. `Module_4_AI_Evaluation`'s five
> teaching files were authored in session 006 from its `README.md` and `AI_Eval_Metrics.ipynb`;
> they have not yet been taught or exercised with a learner, so treat them as unbattle-tested and
> revise as you go.
